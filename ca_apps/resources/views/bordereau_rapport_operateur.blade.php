                                    ...
                                    <table id="buttons-datatables" class="display table table-bordered" style="width:100%">
                                        <thead>
                                            ...
                                            <tr style="background:lightgray;">
                                                <td  colspan="16"><h3 style="text-align:center;">Rapport Action Operateur du  {{\Carbon\Carbon::parse($datedebut)->format('d-m-Y') .' au '.\Carbon\Carbon::parse($datefin)->format('d-m-Y')  }}</h3></td>
                                            </tr>
                                            @else
                                            <tr style="background:lightgray;">
                                                <td  colspan="16"><h3 style="text-align:center;">Rapport Action Operateur  du {{date('d-m-Y')}}</h3></td>
                                            </tr>
                                            @endif
                                            <tr>
                                                 <th>Nom</th>
                                        ...
                                        <tbody>
                                          ...
                                          @foreach ($contrat_condens as $contrat_condens_element)
                                            ...
                                            <tr>
                                                ...
                                                <td>{{count($contrat_condens_element['agent_backoffice']['affectations']) + $contrat_condens_element['totalreaff']}}</td>
                                                <td>{{number_format($contrat_condens_element['totaltraites'], 0, '.', ' ')}}</td>
                                                <td>{{number_format($contrat_condens_element['totalgagne'], 0, '.', ' ')}}</td>
                                            ...
                                            @if (count($contrat_condens_element['agent_backoffice']['affectations'])+ $contrat_condens_element['totalreaff'] !=0)
                                                <td>{{
                                                   number_format( $contrat_condens_element['totaltraites'] * 100 / (count( $contrat_condens_element['agent_backoffice']['affectations'])+ $contrat_condens_element['totalreaff']) ,2)
                                                 . ' % '}} </td>
                                                 @else
                                                     <td>{{
                                                    0 .' %'
                                                 ...
                                                 @if ($contrat_condens_element['totaltraites'] ==0)
                                                     {{-- expr --}}
                                                     <td>0 %</td>
                                                 @else
                                                 <td>{{
                                                    number_format($contrat_condens_element['totalgagne'] * 100 / $contrat_condens_element['totaltraites'] ,2) 
                                                 . ' % '}}  </td>

                                                 @endif

                                                <td>{{ number_format($contrat_condens_element['totalreporte'], 0, '.', ' ');}}</td>
                                                <td>{{ number_format($contrat_condens_element['totalpoursuivre'], 0, '.', ' ');}}</td>
                                                <td>{{ number_format($contrat_condens_element['totalhorscible'], 0, '.', ' ');}}</td>
                                                 <td>{{ number_format($contrat_condens_element['totalrdvsous'], 0, '.', ' ');}}</td>
                                                <td>{{ number_format($contrat_condens_element['totalperdue'], 0, '.', ' ');}}</td>

                                                <td>
                                                   @if ($datedebut !== null  && $datefin !== null)
                                                        {{-- expr --}}
                                                     <a class="btn btn-dark" href="/details_operations_traitees_agent/{{$contrat_condens_element['agent_backoffice']['id'].'/'.$datedebut.'/'.$datefin}}">Details</a>
                                                    
                                                    @else
                                                    <a class="btn btn-dark" href="/details_operations_traitees_agent/{{$contrat_condens_element['agent_backoffice']['id'].'/'.date('Y-m-d').'/'.date('Y-m-d')}}">Details</a>
